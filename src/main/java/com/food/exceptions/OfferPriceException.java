package com.food.exceptions;

@SuppressWarnings("serial")
public class OfferPriceException extends RuntimeException {

	public OfferPriceException() {
		// TODO Auto-generated constructor stub
	}

	public OfferPriceException(String message) {
		super(message);
		// TODO Auto-generated constructor stub
	}

	public OfferPriceException(Throwable cause) {
		super(cause);
		// TODO Auto-generated constructor stub
	}

	public OfferPriceException(String message, Throwable cause) {
		super(message, cause);
		// TODO Auto-generated constructor stub
	}

	public OfferPriceException(String message, Throwable cause, boolean enableSuppression, boolean writableStackTrace) {
		super(message, cause, enableSuppression, writableStackTrace);
		// TODO Auto-generated constructor stub
	}

}
